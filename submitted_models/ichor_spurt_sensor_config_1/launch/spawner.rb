...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='ichor_spurt_description'>
      <command>roslaunch --wait ichor_spurt_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='ichor_spurt_ros_ign_bridge'>
      <command>roslaunch --wait ichor_spurt_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} uav:=1 laser_scan:=0 top_scan:=0 bottom_scan:=0 rgbd_cam:=1    </command>
  </executable>
  HEREDOC
end
